...
def page_admin(session, user):
    ...
    if not can_edit_admin(user):
        st.warning("Apenas Admin_LAG pode alterar dados administrativos.")
    ...
    if APP_ENV == "development":
        with st.expander("Administração avançada"):
            st.warning("Esta ação recria o banco local e apaga os dados existentes.")
